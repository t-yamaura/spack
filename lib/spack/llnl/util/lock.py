...
class OpenFileTracker(object):
    ...
    def get_fh(self, path, op):
        ...
        Arguments:
          ...
          op (int): ``fcntl`` lock operation, e.g., ``fcntl.LOCK_SH``
        """
        ...
        try:
            ...
            if not os.access(path, os.W_OK):
                if op == fcntl.LOCK_SH:
                    ...
                    # can't do LOCK_EX for read-only files
                    raise LockROFileError(path)
...
class Lock(object):
    ...
    def _lock(self, op, timeout=None):
        ...
        if self._file is None:
            ...
            self._file = file_tracker.get_fh(self.path, op)
